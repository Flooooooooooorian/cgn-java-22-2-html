...
const getLivingHumans: (characters: Character[]) => Character[] = (characters) => {
    return characters.filter(character => character.status === "Alive" && character.species === "Human")
}

const getCharacterNames: (characters: Character[]) => string[] = (characters) => {
    return characters.map(char => char.name)
}

const getNameAndOrigin: (characters: Character[]) => { name: string, origin: string }[] = (characters) => {
    return characters.map(char => ({name: char.name, origin: char.origin.name}))
}


console.log(getLivingHumans(characterResponse.results))
console.log(getCharacterNames(characterResponse.results))
console.log(getNameAndOrigin(characterResponse.results))
